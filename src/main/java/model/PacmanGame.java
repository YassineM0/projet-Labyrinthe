 ...
 *
 * Version avec personnage qui peut se deplacer. A completer dans les versions
 * suivantes.
 *
 */
public class PacmanGame implements Game {
    ...
    public PacmanGame(int x, int y, int vie, int attaque, int niveau) {
        this.finished = false;
        this.hero = new Hero(x, y, 100, attaque, false);

        this.tresors = new ArrayList<>();
        genererLabyrinthe();
        this.niveau = 1;
        this.monstres = new ArrayList<>();
        monstres.add(new Monster(13, 13, 70, 1, "Goblin1"));
        monstres.add(new Monster(13, 13, 70, 1, "Goblin2"));
        monstres.add(new Monster(13, 13, 70, 1, "Goblin3"));
        monstres.add(new Monster(13, 13, 70, 1, "Goblin4"));
        monstres.add(new Monster(13, 13, 70, 1, "Goblin5"));
        monstres.add(new Monster(13, 13, 70, 1, "Goblin6"));
        monstres.get(0).setVie(15);
        //  monstres.add(new Monster(6, 6, 10, 3, "Orc"));
        //monstres.add(new Monster(10, 10, 15, 5, "Dragon"));
        this.fantomes = new ArrayList<>();
        fantomes.add(new Fantome(8, 11, 15, 1, "Ghosty"));
    ...
    private void verifierCase() {
        ...
        for (Tresor tresor : tresors) {
            ...
            //System.out.println(labyrinthe[hero.getX()][hero.getY()]);
        ...
        if (!monstres.isEmpty()) {
            for (Monster monstre : monstres) {
                if (hero.getX() == monstre.getX() && hero.getY() == monstre.getY()) {
                    hero.setVie(hero.getVie() - monstre.getAttaque());
                    monstre.setVie(monstre.getVie() - 5);
                    //System.out.println("Collision avec le fantôme : " + monstre.getName());
                    //System.out.println("Vie restante du héros : " + hero.getVie());
                    ...
                    if (monstre.getVie() <= 0) {
                        System.out.println("Le héros est mort !");
                        monstersToRemove.add(monstre); // Terminer le jeu
                    }
                }
            }
            monstres.removeAll(monstersToRemove);
        }
		if (labyrinthe[hero.getX()][hero.getY()] == 6) {
            hero.setX(p.getX1());
			hero.setY(p.getY1());
        }
        ...
        if (labyrinthe[hero.getX()][hero.getY()] == 4) {
            hero.activateMagic();
			labyrinthe[hero.getX()][hero.getY()] = 1;
        }
    ...
    public void evolve(Cmd commande) {
        ...
        if (commande == Cmd.ATTACK) {
            ...
            if (monstreProche != null) {// Trouver un monstre à proximité
                hero.attack(monstreProche);
            }// Attaquer le monstre

        }
		if (commande == Cmd.ATTACK) {
			Monster monstreProche = detecterMonstre();
			if (monstreProche != null) { // Si un monstre est proche, attaque-le
				hero.attack(monstreProche);
			}
		}
        verifierCase();
